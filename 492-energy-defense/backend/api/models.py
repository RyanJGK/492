...
class User(Base):
    ...
    role = Column(SQLEnum(UserRole, values_callable=lambda x: [e.value for e in x]), nullable=False, default=UserRole.OBSERVER)
...
class PatchLevel(Base):
    ...
    severity = Column(SQLEnum(SeverityLevel, values_callable=lambda x: [e.value for e in x]), index=True)
...
class VulnerabilityScan(Base):
    ...
    status = Column(SQLEnum(EventStatus, values_callable=lambda x: [e.value for e in x]), default=EventStatus.PENDING, index=True)
